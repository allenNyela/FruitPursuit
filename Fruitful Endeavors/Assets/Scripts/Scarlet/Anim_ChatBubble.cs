...
public class Anim_ChatBubble : MonoBehaviour
{
    ...
    public float attackDuration   = 1.50f;
    ...
    public float bulletScaleInDuration = 0.4f;
    ...
    void Start()
    {
        ...
        StartCoroutine(BubbleAttackLoop());
    }


    // ── Logic ────────────────────────────────────────────────────────────────

    bool HasBananaTarget()
    {
        if (turret == null || turret.target == null) return false;
        Enemy_Stats stats = turret.target.GetComponentInChildren<Enemy_Stats>();
        return stats != null && stats.isBubbleTarget;
    }

    bool IsBananaShielded() => turret.target.GetComponent<Health>()?.shielded ?? false;

    IEnumerator BubbleAttackLoop()
    {
        while (true)
        {
            if (HasBananaTarget())
            {
                ...
                if (!IsBananaShielded())
                {
                    GetComponent<Bubble_Countdown>()?.StartCountdown();
            ...
            {
                isTargeting = false;
                yield return StartCoroutine(IdleAnim());
            }
        }
    }
    ...
    {
        if (head == null) { yield return null; yield break; }

        Vector3 startScale = head.transform.localScale;
        Vector3 startPos   = head.transform.localPosition;
        float   elapsed    = 0f;

        while (!HasBananaTarget())
        {
            elapsed += Time.deltaTime;
    ...
    void SpawnBullet()
    {
        ...
        Enemy_Stats stats = turret.target.GetComponentInChildren<Enemy_Stats>();
        if (stats == null || !stats.isBubbleTarget) return;
        Health targetHealth = turret.target.GetComponent<Health>();
        if (targetHealth.shielded) return;
        targetHealth.shielded = true;
